...
<template>
  <div 
    class="group bg-card border border-border/40 rounded-lg p-3 hover:border-border/60 transition-all duration-200 hover:shadow-sm"
    ...
    <div class="flex items-start gap-2">
        ...
        <div class="flex items-center gap-2 mb-1">
          <div v-if="editingTaskId === item.id && editingField === 'title'" class="flex-1 flex gap-1">
            <input 
              ...
              class="text-sm font-semibold h-6 px-2 border border-border rounded bg-background focus:ring-1 focus:ring-primary/20" 
          ...
          <div v-else class="flex-1 flex items-center gap-1">
            <span 
              class="text-sm font-semibold cursor-pointer hover:text-primary transition-colors truncate"
              :class="{ 'line-through': item.completed }"
              @click="$emit('startEditing', { field: 'title', value: item.title })"
            >
              {{ item.title }}
            </span>
            <Button size="sm" variant="ghost" @click="$emit('startEditing', { field: 'title', value: item.title })" class="h-4 w-4 p-0 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div class="mb-2">
          <div v-if="editingTaskId === item.id && editingField === 'description'" class="flex gap-1">
            <textarea 
              ...
              class="text-xs px-2 py-1 border border-border rounded bg-background focus:ring-1 focus:ring-primary/20 resize-none flex-1" 
              rows="2"
              @keyup.enter="$emit('saveEdit')"
              @keyup.escape="$emit('cancelEdit')"
      ...
      <div class="flex flex-col gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
        ...
        <Button 
          ...
          variant="ghost" 
          @click="$emit('delete')" 
          class="h-5 w-5 p-0 text-destructive hover:text-destructive hover:bg-destructive/10"
          title="Xóa task"
        >
